...
namespace CtrLayer
{
    public class ReservedNamesController : IReservedNamesController
    {
        ...
        private void CheckAndRemoveExistName() {
            using (BetBudContext db = new BetBudContext()) {
                List<ReservedNames> toRemove = db.ReservedNames.Where(y => DbFunctions.AddMinutes(y.Time, 5) < DateTime.Now).ToList();
